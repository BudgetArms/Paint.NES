...
## Introduction
Paint.NES is painting program, developed for the NES console. It was developed mostly using Mesen the emulator, for tests, but the program was tested weekly on an actual NES as well. And thus, every feature should work as intended.
...
single buttons:
    ...
    A : use tool
    B : change tool properties, eg: brush size, rectangle to circle, etc.
combos:
    Select + left/right D-PAD : select one of the 4 colors
    Select + down/up D-PAD    : change color value of the selected color
...
There are 5 working tools:
    - Brush tool:
        ...
        When pressing B, the cursor size will cycle through 3 sizes. 1x1 tile (1 tile), 2x2 tiles (4 tiles), 3x3 tiles (9 tiles).
    - Eraser tool:
    ...
    The player can freely change any of the 4 colors in the color pallete to any possible color value the NES has.
